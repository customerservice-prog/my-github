...
export async function POST(request:Request){
  ...
  try{
    const project=await one<{id:number}>(`INSERT INTO projects(name,slug,repo_full_name,repo_url,branch,staging_domain,staging_branch,dockerfile,domain,container_port,health_path,server_id)
      ...
      [parsed.data.name,slug,parsed.data.repoFullName,parsed.data.repoUrl,parsed.data.branch,parsed.data.stagingDomain||null,parsed.data.stagingBranch,parsed.data.dockerfile,parsed.data.domain,parsed.data.containerPort,parsed.data.healthPath,parsed.data.serverId]);
